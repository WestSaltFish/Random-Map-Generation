...
{	
	mapWidth = row * tileWidth;
	mapHeight = col * tileHeight;
}
...
{
	if (mapPos.x < 0 || mapPos.y < 0 || mapPos.x >(row - 1) || mapPos.y >(col - 1)) return -1;

	return tiles[mapPos.y * row + mapPos.x].type;
}
...
{
	if (mapPos.x <= 0 || mapPos.y <= 0 || mapPos.x >= (row - 1) || mapPos.y >= (col - 1)) return true;
...
void Map::PostUpdate()
{
	...
	{
		Tile t = tiles[posibleDir_t[i].y * row + posibleDir_t[i].x];
